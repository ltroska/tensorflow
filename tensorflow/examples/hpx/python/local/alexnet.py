#taken from https://github.com/soumith/convnet-benchmarks/
#to use with HPX in local mode, note the creation of the session
#using tf.Session('local_hpx')
...
tf.app.flags.DEFINE_boolean('with_hpx', False, """Use HPX""")

tf.app.flags.DEFINE_integer('num_threads', 32, """Number of threads""")
...
def store_data_in_csv(timing_entries):
  with open(FLAGS.csv_file, 'ab') as csvfile:
...
def run_benchmark():
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.with_hpx:
        sess = tf.Session('local_hpx', config=tf.ConfigProto(intra_op_parallelism_threads=FLAGS.num_threads))
    else:
        sess = tf.Session('', config=tf.ConfigProto(intra_op_parallelism_threads=FLAGS.num_threads))
    sess.run(init)
