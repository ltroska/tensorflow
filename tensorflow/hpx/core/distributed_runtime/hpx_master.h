...
struct HPXMaster : public MasterInterface
{
    ...
    : init_(rt), basename_("master:" + basename)
    {
      MaybeRunAsHPXThread(
        [this, master]() {
          client_ = 
            hpx::id_type(hpx::components::server::construct
                            <hpx::components::component<server::HPXMasterServer> >
                            (master)
                          ,hpx::id_type::managed);
    ...
    HPXMaster(HPXMaster const& other) : client_(other.client_), init_(other.init_), basename_(other.basename_) {}
    HPXMaster(HPXMaster&& other) : client_(std::move(other.client_)), init_(other.init_), basename_(std::move(other.basename_)) {}
    ...
    Status CreateSession(CallOptions* call_options,
      ...
      return CallAction(&HPXMasterClient::CreateSession, "CreateSession", call_options, request, response);
    }

    Status ExtendSession(CallOptions* call_options,
                         const ExtendSessionRequest* request,
                         ExtendSessionResponse* response) override {
      return CallAction(&HPXMasterClient::ExtendSession, "ExtendSession", call_options, request, response); 
    }

    Status PartialRunSetup(CallOptions* call_options,
                           const PartialRunSetupRequest* request,
                           PartialRunSetupResponse* response) override {
      return CallAction(&HPXMasterClient::PartialRunSetup, "PartialRunSetup", call_options, request, response); 
    }

    Status RunStep(CallOptions* call_options, RunStepRequestWrapper* request,
                   MutableRunStepResponseWrapper* response) override {
      return CallAction(&HPXMasterClient::RunStep, "RunStep", call_options, &request->ToProto(), get_proto_from_wrapper(response));
    }

    Status CloseSession(CallOptions* call_options,
                        const CloseSessionRequest* request,
                        CloseSessionResponse* response) override {
      return CallAction(&HPXMasterClient::CloseSession, "CloseSession", call_options, request, response);
    }

    Status ListDevices(CallOptions* call_options,
                       const ListDevicesRequest* request,
                       ListDevicesResponse* response) override {
      return CallAction(&HPXMasterClient::ListDevices, "ListDevices", call_options, request, response);      
    }

    Status Reset(CallOptions* call_options, const ResetRequest* request,
                 ResetResponse* response) override {
      return CallAction(&HPXMasterClient::Reset, "Reset", call_options, request, response);      
    }
...
protected:
    ...
    inline Status
    CallAction(F&& f, char const* reg_string, Args... args)
    {
      return MaybeRunAsHPXThread(
        [this, f, args...] () -> Status
        {
...
inline HPXMaster* NewHPXMaster(global_runtime* init, std::string const& basename)
{
