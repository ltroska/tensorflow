#include "tensorflow/hpx/common_runtime/hpx_direct_session.h"
...
namespace
{
  ...
  auto* direct_session_runs = monitoring::Counter<0>::New(
      "/tensorflow/hpx/hpx_direct_session_runs",
      "The number of times HPXDirectSession::Run() has been called.");
  ...
  thread::ThreadPool*
  NewThreadPoolFromSessionOptions(const SessionOptions& options)
  {
  ...
  thread::ThreadPool* NewThreadPoolFromThreadPoolOptions(
      ...
      const ThreadPoolOptionProto& thread_pool_options,
      int pool_number)
  {
...
class HPXDirectSessionFactory : public SessionFactory
{
  public:
  HPXDirectSessionFactory()
  {
  }
  ...
  Session* NewSession(const SessionOptions& options) override
  {
    ...
    HPXDirectSession* session =
        new HPXDirectSession(options, new DeviceMgr(devices), this);
    {
  ...
  {
    std::vector<HPXDirectSession*> sessions_to_reset;
    {
      mutex_lock l(sessions_lock_);
      // We create a copy to ensure that we don't have a deadlock when
      // session->Close calls the HPXDirectSessionFactory.Deregister, which
  ...
  void Deregister(const HPXDirectSession* session)
  {
    mutex_lock l(sessions_lock_);
  ...
  std::vector<HPXDirectSession*> sessions_ GUARDED_BY(sessions_lock_);
};

class HPXDirectSessionRegistrar
{
  public:
  HPXDirectSessionRegistrar()
  {
    SessionFactory::Register("HPX_DIRECT_SESSION",
                             new HPXDirectSessionFactory());
  }
};
static HPXDirectSessionRegistrar registrar;

std::atomic_int_fast64_t HPXDirectSession::step_id_counter_(1);
...
void HPXDirectSession::SchedClosure(thread::ThreadPool* pool,
                                    std::function<void()> c)
{
...
HPXDirectSession::HPXDirectSession(const SessionOptions& options,
                                   const DeviceMgr* device_mgr,
                                   HPXDirectSessionFactory* const factory)
    : options_(options)
    ...
    , operation_timeout_in_ms_(options_.config.operation_timeout_in_ms())
{
  ...
  // handle, because HPXDirectSession owns its devices. This may change
  // in future versions.
...
HPXDirectSession::~HPXDirectSession()
{
  if (!closed_)
    Close().IgnoreError();
  ...
  init.stop_and_shutdown();
}

Status
HPXDirectSession::MaybeInitializeExecutionState(const GraphDef& graph,
...
Status HPXDirectSession::Create(const GraphDef& graph)
{
...
Status HPXDirectSession::Extend(const GraphDef& graph)
{
...
Status HPXDirectSession::ExtendLocked(const GraphDef& graph)
{
...
Status HPXDirectSession::Run(const NamedTensorList& inputs,
                             ...
                             std::vector<Tensor>* outputs)
{
  ...
  return Run(
      RunOptions(), inputs, output_names, target_nodes, outputs, &run_metadata);
}

Status HPXDirectSession::Run(const RunOptions& run_options,
                             ...
                             RunMetadata* run_metadata)
{
  ...
  TF_RETURN_IF_ERROR(GetOrCreateExecutors(pool,
                                          input_tensor_names,
                                          output_names,
                                          target_nodes,
                                          &executors_and_keys,
  ...
  bool already_cancelled =
      !cancellation_manager_->RegisterCallback(cancellation_token,
...
Status HPXDirectSession::PRunSetup(const std::vector<string>& input_names,
                                   const std::vector<string>& output_names,
                                   const std::vector<string>& target_nodes,
                                   string* handle)
{
  ...
  {
    mutex_lock l(executor_lock_);
    if (!partial_runs_.emplace(run_state_args.handle,
                               std::unique_ptr<RunState>(run_state)).second) {
      return errors::Internal("The handle '",
...
Status HPXDirectSession::PRun(const string& handle,
                              const NamedTensorList& inputs,
                              const std::vector<string>& output_names,
                              std::vector<Tensor>* outputs)
{
  ...
  RunState* run_state;
  {
    ...
    for (const auto& input : inputs) {
      ...
      if (it == run_state->pending_inputs.end()) {
        return errors::InvalidArgument(
            "The feed ", input.first, " had already been fed.");
      }
    }
    ...
    for (const auto& output : output_names) {
      ...
      if (it == run_state->pending_outputs.end()) {
        return errors::InvalidArgument(
            "The fetch ", output, " had already been fetched.");
      }
    }
  }
...

  {
    ...
    if (done) {
      WaitForNotification(
          run_state, cancellation_manager_, operation_timeout_in_ms_);
...
Status HPXDirectSession::SendInputs(const NamedTensorList& inputs,
...
Status HPXDirectSession::RecvOutputs(const std::vector<string>& output_names,
                                     const ExecutorsAndKeys* executors_and_keys,
                                     RunState* run_state,
                                     std::vector<Tensor>* outputs)
{
  ...
  for (size_t output_offset = 0; output_offset < output_names.size();
    ...
    if (it == executors_and_keys->output_keys.end()) {
      return errors::Internal(
          "'", output_name, "' is not a pre-defined fetch.");
    }
...
Status HPXDirectSession::CheckFetch(const NamedTensorList& feeds,
...
Status
HPXDirectSession::GetOrCreateExecutors(thread::ThreadPool* pool,
                                       ...
                                       RunStateArgs* run_state_args)
{
  ...
  for (auto iter = graphs.begin(); iter != graphs.end(); ++iter) {
    ...
    item->flib.reset(NewFunctionLibraryRuntime(device_mgr_.get(),
                                               options_.env,
                                               device,
                                               graph_def_version,
                                               ek->flib_def.get(),
...
Status HPXDirectSession::CreateGraphs(
    ...
    RunStateArgs* run_state_args)
{
  ...
  if (options_.config.graph_options().place_pruned_graph()) {
    ...
    TF_RETURN_IF_ERROR(SimpleGraphExecutionState::MakeForPrunedGraph(
        execution_state_->original_graph_def().library(),
        prune_options,
  ...
  for (auto placement_pair : current_stateful_placements) {
    ...
    if (iter == stateful_placements_.end()) {
      ...
      return errors::Internal("Stateful placement mismatch. "
  ...
  for (const auto& partition : partitions) {
    ...
    if (std::count(device_names.begin(),
                   device_names.end(),
  ...
  for (const auto& partition : partitions) {
    ...
    TF_RETURN_IF_ERROR(ConvertGraphDefToGraph(
        device_opts, partition.second, device_graph.get()));
...
::tensorflow::Status
HPXDirectSession::Reset(const std::vector<string>& containers)
{
  device_mgr_->ClearContainers(containers);
  return ::tensorflow::Status::OK();
}

::tensorflow::Status HPXDirectSession::Close()
{
...
HPXDirectSession::RunState::RunState(
...
HPXDirectSession::RunState::RunState(int64 step_id,
                                     const std::vector<Device*>* devices)
...
void HPXDirectSession::WaitForNotification(RunState* run_state,
...
::tensorflow::Status
HPXDirectSession::WaitForNotification(Notification* notification,
                                      int64 timeout_in_ms)
{
  if (timeout_in_ms > 0) {
