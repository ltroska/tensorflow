#ifndef TENSORFLOW_HPX_COMMON_RUNTIME_HPX_DIRECT_SESSION_H_
#define TENSORFLOW_HPX_COMMON_RUNTIME_HPX_DIRECT_SESSION_H_
...
class DebugGateway;
class Device;
class HPXDirectSessionFactory;

class HPXDirectSession : public Session
{
  public:
  typedef std::function<void(Session*)> CloseCallback;

  // Takes ownership of 'device_mgr'.
  // 'factory' is used to unregister the HPXDirectSession with 'factory' when
  // its
  // closed. This ensures that Reset requests from the 'factory' don't get sent
  // to sessions that are already closed.
  HPXDirectSession(const SessionOptions& options,
                   const DeviceMgr* device_mgr,
                   HPXDirectSessionFactory* factory);
  ~HPXDirectSession() override;
  ...
  // Reset clears 'containers' from the device_mgr of the HPXDirectSession.
  ...
  ::tensorflow::Status
  GetOrCreateExecutors(thread::ThreadPool* pool,
  ...
  ::tensorflow::Status
  SendInputs(const std::vector<std::pair<string, Tensor> >& inputs,
  ...
  ::tensorflow::Status
  CheckFetch(const std::vector<std::pair<string, Tensor> >& feeds,
  ...
  ::tensorflow::Status WaitForNotification(Notification* n,
                                           int64 timeout_in_ms);
  void WaitForNotification(RunState* run_state,
  ...
  HPXDirectSessionFactory* const factory_; // not owned
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(HPXDirectSession);
...
#endif // TENSORFLOW_HPX_COMMON_RUNTIME_HPX_DIRECT_SESSION_H_
